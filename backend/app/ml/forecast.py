...
def _run_fallback_forecast(daily_sales: pd.DataFrame, periods: int) -> ForecastResponse:
    ...
    daily_sales['weekday'] = daily_sales['ds'].dt.weekday
...
def generate_forecast(db: Session, periods: int = 30, upload_id: int | None = None) -> ForecastResponse:
    ...
    # Convert ds back to datetime (Prophet requires datetime or Timestamp, not datetime.date)
    daily_sales['ds'] = pd.to_datetime(daily_sales['ds'])

    if Prophet is None or len(daily_sales) < 5:
        return _run_fallback_forecast(daily_sales, periods)
    ...
    try:
        ...
        print(f"Prophet forecast failed: {e}. Falling back to seasonal naïve forecaster.")
        return _run_fallback_forecast(daily_sales, periods)
